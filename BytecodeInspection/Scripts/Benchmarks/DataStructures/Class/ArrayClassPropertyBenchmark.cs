...
using BenchmarkClass = BytecodeInspection.Benchmarks.SaticArrayClassPropertyBenchmark;

namespace BytecodeInspection.Benchmarks
{
    public class ArrayClassPropertyBenchmark : Benchmark
    {
        private ArrayClassProperty<int> m_array;
        ...
        {
            var sum = 0;
            for(int i = m_array.Items.Length - 1; i >= 0; i--)
            {
                sum += m_array.Items[i];
            }
            return sum;
